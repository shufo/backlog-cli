...
func PrintIssue(param *PrintIssueParams) {
	...
	// if output redirected to other than stdout
...
func printPlain(param *PrintIssueParams) {
	// print summary
	fmt.Printf("%s ", *param.Issue.Summary)
	...
	fmt.Printf(
		"%s %s %s opened about %s\n",
		fmt.Sprintf("状態:"),
		fmt.Sprint(*param.Issue.Status.Name),
		*param.Issue.CreatedUser.Name,
		TimeDiffString(*param.Issue.Created),
	)
	...
	// print issue type
	fmt.Printf("%s %s\n", "種別:", *param.Issue.IssueType.Name)

	// print issue priority
	fmt.Printf("%s %s\n", "優先度:", *param.Issue.Priority.Name)
	...
	if param.Issue.DueDate != nil {
		...
		fmt.Printf("%s %s\n", "期限日:", parsed.Format("2006/01/02"))
	}

	fmt.Println("")

	fmt.Print(*param.Issue.Description)
